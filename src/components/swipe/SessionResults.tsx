    ...
    <div className="min-h-screen bg-bg-primary flex flex-col">
      ...
      >
        ...
        <motion.h1
          ...
          className="text-2xl font-bold font-display text-text-primary mb-1"
        >
          Session <span className="gradient-text-primary">Complete!</span>
        </motion.h1>
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      <motion.div
        ...
        className="flex justify-center gap-3 px-4 mb-6"
      >
        <div className="bg-bg-secondary/80 rounded-2xl px-5 py-3 text-center border border-white/[0.04] shadow-md">
          <div className="text-2xl font-bold font-display text-accent-primary">
            {matches.length}
          </div>
          <div className="text-[11px] text-text-muted">Matches</div>
        </div>
        <div className="bg-bg-secondary/80 rounded-2xl px-5 py-3 text-center border border-white/[0.04] shadow-md">
          <div className="text-2xl font-bold font-display text-text-primary">
            {totalSwiped}
          </div>
          <div className="text-[11px] text-text-muted">Reviewed</div>
        </div>
        <div className="bg-bg-secondary/80 rounded-2xl px-5 py-3 text-center border border-white/[0.04] shadow-md">
          <div className="text-2xl font-bold font-display text-accent-secondary">
            {matchRate}%
          </div>
          <div className="text-[11px] text-text-muted">Match Rate</div>
        </div>
      </motion.div>
        ...
        {matches.length === 0 ? (
          ...
          >
            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-accent-primary/10 flex items-center justify-center">
              <Frown className="w-8 h-8 text-accent-primary" />
            </div>
          ...
          <>
            <motion.h2
              ...
              animate={{ opacity: 1 }}
              transition={{ delay: 0.6 }}
              className="text-lg font-semibold font-display text-text-primary mb-4 flex items-center gap-2"
            >
              <Heart
            ...
            <div className="space-y-4">
              {matches.map((restaurant, index) => (
                <motion.div
                  ...
                  className="bg-bg-secondary/90 rounded-3xl overflow-hidden border border-white/[0.04] hover:border-accent-primary/30 transition-all duration-300 shadow-xl"
                >
                  ...
                  >
                    ...
                    {/* Rating Badge */}
                    {restaurant.rating && (
                      <div className="absolute top-3 right-3 bg-black/60 backdrop-blur-sm rounded-xl px-2.5 py-1 flex items-center gap-1 border border-white/10">
                        <Star
                    ...
                    {/* Match Badge */}
                    <div className="absolute top-3 left-3 bg-accent-primary text-white backdrop-blur-sm rounded-xl px-2.5 py-1 flex items-center gap-1 shadow-lg shadow-accent-primary/30">
                      <Heart
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-3 flex-wrap mb-3">
                      ...
                      {restaurant.priceLevel && (
                        <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-accent-primary/15 text-accent-primary font-display">
                          {restaurant.priceLevel}
                        </span>
                      )}
                      {/* Distance */}
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 1 }}
        className="sticky bottom-0 p-4 bg-bg-primary/80 backdrop-blur-lg border-t border-white/[0.04] space-y-3"
      >
        <Link href="/" className="block">
          <button className="w-full py-3.5 rounded-2xl font-semibold font-display text-white bg-gradient-to-r from-accent-primary to-[#d4284a] hover:shadow-[0_8px_32px_rgba(255,58,92,0.3)] transition-all duration-300">
            Start New Session
          </button>
        </Link>
